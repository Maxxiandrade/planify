...
import { addService, changeProgress, setProgress } from "../store/features/serviceSlice";
        ...
        <form action="" className="w-full">
          {Object.entries(organizedServices).map(([category, services]) => (
            <div key={category}>
              ...
              {expandedServices[category] && services.map(servicio => (
                <div key={servicio.id} className="border border-stone-300 w-11/12 flex flex-col items-start mb-2 my-3 ml-5 rounded-sm">
                    ...
                    <label
                      ...
                      className={`${
                        selectedService === servicio.id
                          ? "bg-slate-600 rounded-sm mr-3 mb-3 px-2 py-1 text-white"
                          : "bg-slate-500 rounded-sm mr-3 mb-3 px-2 py-1 text-white"
                      }`}
                    >
                      <input
                        type="radio"
